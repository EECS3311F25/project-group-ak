...
class TripRouteTests {
    ...
    fun lmao_1() = testApplication {
         ...
         */
        val tripDeleteResponse: HttpResponse = client.delete("/user/2/trip/2") {
        ...
        val userDeleteResponse: HttpResponse = client.delete("/user/2/delete") {
            contentType(ContentType.Application.Json)
        }
    ...
    fun tripCreate_TestSuccess_1() = testApplication {
        ...
        println("\n" + tripUpdateResponse.bodyAsText() + "\n")

        val tripUpdateResBody = tripUpdateResponse.body<TripRetrieveResponse>()
        assertEquals("Trip created successfully", tripUpdateResBody.message)

        val tripUpdateResData = tripUpdateResBody.data
        assertEquals("Updated Trip 1", tripUpdateResData.tripTitle)
        assertEquals("Updated Trip Description 1", tripUpdateResData.tripDescription)
        assertEquals("Updated Location 1", tripUpdateResData.tripLocation)
        assertEquals(Duration(
            startDate = LocalDate(2026, Month.MARCH, 21),
            startTime = LocalTime(hour = 8, minute = 30, second = 16),
            endDate = LocalDate(2026, Month.MARCH, 28),
            endTime = LocalTime(hour = 8, minute = 30, second = 16)),
            tripUpdateResData.tripDuration)
